...
export class SigninRequestDto {
    @ApiProperty({ example: 'usuario@email.com', description: 'E-mail utilizado como username'})
    ...
    @Matches(
        /^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*#?&])[A-Za-z\d@$!%*#?&]{6,}$/,
        { message: 'O Password deve conter letras, números e ao menos 1 caracter especial' }
    )
    password!: string;
